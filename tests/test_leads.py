...
def make_repo(email_exists: bool = False) -> AsyncMock:
    repo = AsyncMock()
...
@pytest.mark.asyncio
async def test_capture_new_lead_inserts():
    repo = make_repo(email_exists=False)
    svc = LeadService(repo)
    result = await svc.capture(
        LeadRequest(email="new@example.com", name="Ali", company="Pharma Co")
    )
...
@pytest.mark.asyncio
async def test_capture_duplicate_does_not_insert():
    repo = make_repo(email_exists=True)
    svc = LeadService(repo)
    result = await svc.capture(LeadRequest(email="dup@example.com"))
    assert result.success is True
