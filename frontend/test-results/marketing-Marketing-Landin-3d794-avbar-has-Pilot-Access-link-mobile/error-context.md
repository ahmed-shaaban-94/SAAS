...
- Name: marketing.spec.ts >> Marketing Landing Page — Pharma-First >> navbar has Pilot Access link
...
Locator: locator('nav').getByRole('link', { name: /Pilot Access/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('nav').getByRole('link', { name: /Pilot Access/i }).first()
