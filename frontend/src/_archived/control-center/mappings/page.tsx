...
/** /control-center/mappings — Column mapping templates on the v2 shell. */
...
function MappingForm({
  ...
  const [columns, setColumns] = useState<MappingColumn[]>(
    existingCols.length ? existingCols : [{ source: "", canonical: "", cast: "string" }],
  );
    ...
    <div className="rounded-2xl border border-border/50 bg-card p-6">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-base font-semibold text-text-primary">
          {initial ? "Edit Mapping" : "New Mapping Template"}
        </h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!initial && (
            <div>
              ...
              <select
                ...
                className="w-full rounded-xl border border-border/70 bg-background/60 px-3 py-2 text-sm text-text-primary"
              >
                {["file_upload", "google_sheets", "postgres"].map((t) => (
        ...
        <div>
          <div className="mb-2 flex items-center justify-between">
            ...
            <button type="button" onClick={addCol} className="text-xs text-primary hover:underline">
              + Add row
            </button>
          </div>
        ...
        {validation && (
          <div
            className={`rounded-xl p-4 text-sm ${
              validation.ok ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"
            }`}
          >
        ...
        <div className="flex justify-end gap-2">
          <button
            ...
            disabled={validating}
            className="rounded-xl border border-border/70 px-4 py-2 text-sm text-text-secondary hover:text-text-primary disabled:opacity-60"
          >
          ...
          <button
            type="button"
            onClick={onCancel}
            className="rounded-xl border border-border/70 px-4 py-2 text-sm text-text-secondary"
          >
            Cancel
          </button>
          <button
...
export default function MappingsPage() {
  ...
  return (
    <DashboardShell
      activeHref="/control-center/mappings"
      breadcrumbs={[
    ...
    >
      <div className="page">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="page-title">Mapping templates.</h1>
            <p className="page-sub">Column-level source → canonical mapping definitions.</p>
          </div>
          <button
            onClick={() => {
              setEditing(null);
        ...
        {isLoading && !data.items.length ? (
          ...
          <ErrorRetry title="Failed to load mappings" />
        ) : (
