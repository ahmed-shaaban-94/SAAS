...
/** /control-center/profiles — Pipeline profiles on the v2 shell. */
    ...
    <div className="rounded-2xl border border-border/50 bg-card p-6">
      <div className="mb-4 flex items-center justify-between">
        <h3 className="text-base font-semibold text-text-primary">
          {initial ? "Edit Profile" : "New Pipeline Profile"}
        </h3>
...
export default function ProfilesPage() {
  ...
  return (
    <DashboardShell
      activeHref="/control-center/profiles"
      breadcrumbs={[
    ...
    >
      <div className="page">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="page-title">Pipeline profiles.</h1>
            <p className="page-sub">Processing profiles targeting canonical data domains.</p>
          </div>
          <button
            onClick={() => {
              setEditing(null);
        ...
        {isLoading && !data.items.length ? (
          ...
          <ErrorRetry title="Failed to load profiles" />
        ) : (
            ...
            {data.items.map((profile) => (
              <div key={profile.id} className="rounded-2xl border border-border/50 bg-card p-5 space-y-2">
                ...
                <p className="text-xs text-text-secondary">
                  Domain: <strong className="text-text-primary">{profile.target_domain}</strong>
                </p>
