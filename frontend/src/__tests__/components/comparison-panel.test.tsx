...
// Pin the contract: comparison-panel must read period totals (today_gross /
// daily_transactions), NOT mtd_* fields. This test would have caught the
...
describe("ComparisonPanel", () => {
  it("reads period totals (today_gross / daily_transactions), not mtd_* fields", () => {
    render(<ComparisonPanel onClose={() => {}} />);

    // Revenue must come from today_gross (1,500,000), not mtd_gross (0).
    ...
    // Transactions must come from daily_transactions (2,400), not
    // mtd_transactions (0).
  ...
  it("does NOT render zero when mtd_gross is zero but today_gross has data", () => {
    render(<ComparisonPanel onClose={() => {}} />);
    const revenueCell = screen.getByText("Revenue").closest("div");
