...
### Security
- **Authentication**: Clerk is the sole IdP. Backend JWT verification reads `active_jwks_url` / `active_issuer_url` / `active_audience` from `core/config.py`. Frontend uses `@clerk/nextjs` via the bridge at `frontend/src/lib/auth-bridge.tsx`. The Clerk JWT template named `datapulse` must emit `tenant_id` + `roles` claims (see `scripts/clerk_setup.py`).
