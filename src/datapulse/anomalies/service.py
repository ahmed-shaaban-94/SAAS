...
class AnomalyService:
    ...
    def get_cards(self, limit: int = 10) -> list[AnomalyCard]:
        """Return active alerts projected to the design-handoff card shape.

        Powers the dashboard anomaly feed — see issue #508.
        """
        alerts = self._repo.get_active_alerts(limit=limit)
        return [to_card(a) for a in alerts]
