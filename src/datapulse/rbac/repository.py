...
class RBACRepository:
    ...
    def list_roles(self) -> list[dict]:
        rows = self._s.execute(
            text("SELECT role_id, role_key, role_name, description, is_system FROM public.roles ORDER BY role_id")
        ).mappings().all()
        return [dict(r) for r in rows]

    def get_role_by_key(self, role_key: str) -> dict | None:
        row = self._s.execute(
            text("SELECT role_id, role_key, role_name, description, is_system FROM public.roles WHERE role_key = :rk"),
            {"rk": role_key},
        ).mappings().first()
    ...
    def list_permissions(self) -> list[dict]:
        rows = self._s.execute(
            text("SELECT permission_key, category, description FROM public.permissions ORDER BY category, permission_key")
        ).mappings().all()
        return [dict(r) for r in rows]
    ...
    def accept_invite(self, member_id: int, user_id: str) -> dict | None:
        self._s.execute(
            ...
            """),
            {"mid": member_id, "uid": user_id, "now": datetime.now(timezone.utc)},
        )
        return self.get_member_by_id(member_id)
