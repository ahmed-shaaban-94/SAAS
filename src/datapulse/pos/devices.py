...
async def device_token_verifier(
    ...
    """
    # Fail fast on missing tenant context. A silent fallback to tenant_id=1
    # would let a device-fingerprint-v2 check run against the wrong tenant's
    # terminal_devices row on a middleware regression.
    try:
