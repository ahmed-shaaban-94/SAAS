"""Excel receipts loader — loads stock receipts Excel files into bronze.stock_receipts."""
...
class ExcelReceiptsLoader(BronzeLoader):
    """Loads stock receipt records from Excel files into bronze.stock_receipts.
    ...
    def read(self, source: Path) -> pl.DataFrame:
        ...
        log.info("reading_receipts_file", file=source.name)
    ...
    def validate(self, df: pl.DataFrame) -> pl.DataFrame:
        ...
        missing = {"drug_code", "receipt_date"} - set(df.columns)
        if missing:
