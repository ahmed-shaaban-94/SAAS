...
> These rules describe the module dependency constraints for the DataPulse codebase. Violations should be fixed before merging.
...
- Business modules (**analytics/**, **pipeline/**, **forecasting/**, **billing/**, **ai_light/**, **explore/**) may import from `core/`, `cache`, and `config` — never from `api/`.
- **core/** and `cache` may only import from `config`.
- **bronze/** and **import_pipeline/** are leaf modules — import only from `config`, stdlib, and third-party.
...
3. **Repository** (`<module>/repository.py`) — raw SQL via SQLAlchemy. No business logic.
...
- Use `get_tenant_session` (authenticated, RLS-scoped) — never the deprecated `get_db_session`.
